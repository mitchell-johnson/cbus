...
| SAL applications | Implemented | Lighting, AIRCON/HVAC, Audio, Security, Measurement, Telephony, clock, enable control, temperature broadcast, and status request |
...
| Embedded physical C-Gate | In progress | Persistent database plus real lighting; complete maintained direct-network AIRCON, Audio, Security, Measurement and Telephony command families; all 128 maintained DALI paths routed as 103 physical leaves and 25 local/help paths, with exact physical `EXT_ONLY` commissioning and a narrower pre-I/O boundary for unevidenced typed-device session plans; native `DBNETWORKPATH`; local PROJECT help/DIRFULL, NAC DBGETJSON projections, event-channel subscriptions and advisory locks; shared-interface `PROJECT_IDENTIFY`; direct and one-to-six-bridge read-only `PINGU`/`SYNC`/`CHECKUNIT`, general `SYNCNEW`, and `DO ... SYNC`; physical network-clock inspection/configuration/recovery; persistent named-scene record/playback; Trigger, Enable, clock, Temperature Broadcast, dynamic labels, eDLT/KFI controls, direct targeted `SYNCNEW`, verified `SET_PROJECT_IDENTIFY`, guarded readdressing and bounded `UNRAVELUNIT MATCHDB`, unit identity, physical PP LOAD/SAVE with native C-Bus 3 NVM commit, memory backends, local PP catalogue/raw administration, and runtime PROGRAMMER queues over cmqttd's shared CNI connection. PROGRAMMER START and PP WRITE_PATCH remain fail-closed |
