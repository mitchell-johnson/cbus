...
| Embedded physical C-Gate | In progress | Persistent database plus real lighting; complete maintained direct-network AIRCON, Audio, Security, Measurement, Media Transport and Telephony families plus Identify control, Short Message and Error Reporting leaves; all 128 maintained DALI paths routed as 103 physical leaves and 25 local/help paths, with exact physical `EXT_ONLY` commissioning and a narrower pre-I/O boundary for unevidenced typed-device session plans; native `DBNETWORKPATH`; durable local NET catalogue lifecycle, exact NET/NETWORK/TOPOLOGY help, direct physical `NET LEARN` and `NETWORK LOCATE`; native-shaped comments and OID, native timestamped `BROADCAST_EVENT` fanout, exact object/application SHOW matrices, parser grammar, TREE/XML/REPORT hierarchy, and bounded durable database-only NEW objects; local PROJECT help/DIRFULL, NAC DBGETJSON projections, event-channel subscriptions and advisory locks; shared-interface `PROJECT_IDENTIFY`; direct and one-to-six-bridge read-only `PINGU`/`SYNC`/`CHECKUNIT`, general `SYNCNEW`, and `DO ... SYNC`; physical network-clock inspection/configuration/recovery; persistent named-scene record/playback; Trigger, Enable, clock, Temperature Broadcast, dynamic labels, eDLT/KFI controls, direct targeted `SYNCNEW`, verified `SET_PROJECT_IDENTIFY`, guarded readdressing and bounded `UNRAVELUNIT MATCHDB`, unit identity, physical PP LOAD/SAVE with native C-Bus 3 NVM commit, memory backends, local PP catalogue/raw administration, the bounded local DEPLOY_QUEUE lifecycle, and runtime PROGRAMMER queues over cmqttd's shared CNI connection. Short Message SEND is an explicit repair of a retained native malformed encoder. PROGRAMMER START and PP WRITE_PATCH remain fail-closed |
