...
| Embedded physical C-Gate | In progress | Persistent database plus real lighting, native `DBNETWORKPATH`, direct and one-to-six-bridge read-only `PINGU`/`SYNC`/`CHECKUNIT` and `DO ... SYNC`, physical network-clock inspection/configuration/recovery, persistent named-scene record/playback, Trigger, Enable, clock, Temperature Broadcast, dynamic labels, eDLT/KFI controls, direct-network `SYNCNEW`, verified `SET_PROJECT_IDENTIFY`, guarded readdressing and bounded `UNRAVELUNIT MATCHDB`, unit identity, physical PP LOAD/SAVE with native C-Bus 3 NVM commit, and memory backends over cmqttd's shared CNI connection |
