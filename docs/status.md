...
| Embedded physical C-Gate | In progress | Persistent database plus real lighting, Trigger, Enable, clock, Temperature Broadcast, complete-coverage PINGU discovery, identity-populating SYNC, duplicate-aware CHECKUNIT, guarded physical unit readdressing, identity, physical PP LOAD/SAVE, and memory backends over cmqttd's shared CNI connection |
