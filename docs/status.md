...
| Embedded physical C-Gate | In progress | Persistent database plus real lighting; complete maintained direct-network AIRCON, Audio, Security, Measurement and Telephony command families; 62 physical DALI core/emergency leaves and six local DALI help roots, with 60 specialized leaves still fail-closed; native `DBNETWORKPATH`; local PROJECT help/DIRFULL, NAC DBGETJSON projections, event-channel subscriptions and advisory locks; shared-interface `PROJECT_IDENTIFY`; direct and one-to-six-bridge read-only `PINGU`/`SYNC`/`CHECKUNIT`, general `SYNCNEW`, and `DO ... SYNC`; physical network-clock inspection/configuration/recovery; persistent named-scene record/playback; Trigger, Enable, clock, Temperature Broadcast, dynamic labels, eDLT/KFI controls, direct targeted `SYNCNEW`, verified `SET_PROJECT_IDENTIFY`, guarded readdressing and bounded `UNRAVELUNIT MATCHDB`, unit identity, physical PP LOAD/SAVE with native C-Bus 3 NVM commit, and memory backends over cmqttd's shared CNI connection |
