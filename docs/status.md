...
| Embedded physical C-Gate | In progress | Persistent database plus real lighting, physical `DO` lighting and direct-network `SYNC` methods, physical network-clock inspection/configuration/recovery, persistent named-scene record/playback, Trigger, Enable, clock, Temperature Broadcast, native dynamic-label broadcasts, standard `LABEL CLEAR`, bounded observed label-SAL assembly, eDLT clear control, physical KFI commands, complete-coverage PINGU discovery, identity-populating `SYNC` with native KEYGL5 extended FirmwareVersion, Application/Application2 and static WidgetGroups readback, five-pass `SYNCNEW`, duplicate-aware `CHECKUNIT`, verified `SET_PROJECT_IDENTIFY`, guarded physical unit readdressing, the bounded two-unit source-255 `UNRAVELUNIT MATCHDB` workflow, unit identity, physical PP LOAD/SAVE with native C-Bus 3 NVM commit, and memory backends over cmqttd's shared CNI connection |
