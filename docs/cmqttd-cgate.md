...
| NET SYNC and cached unit getters | Configured interface routing hint (physically revalidated) or BASIC discovery, complete installation MMI, then confirmed IDENTIFY1/2 probes and bounded IDENTIFY4 collection for every present address; routed and local bare-CAL replies are correlated, silent legacy/error addresses remain present with unknown identity fields, the live cache is replaced atomically, and native getters expose it. Native KEYGL5 metadata additionally requires configured and fresh KEYGL5 types, non-error present MMI state one or two, and exactly one raw IDENTIFY4 reply carrying a known serial. Only then does cmqttd run the metadata sequence over the captured OEM `09 00` route: parameter `0xFB` length 9 for NUL-terminated `FirmwareVersion`, OEM address 16 length 2 for decimal `Application`/`Application2`, then parameter `0xFA` length 44 for opaque decimal-CSV `WidgetGroups`. Ineligible addresses, including repeated identical known replies and mixed known/unknown replies, receive no metadata traffic and stale values are removed. Cached `GET` issues no bus I/O and `Version` remains IDENTIFY2. An address with multiple distinct serials emits `#e# net {network} sync duplicate {address} {serial...}` (sorted); scalar `SerialNumber` stays `""` while the sorted set of distinct serials is retained in the volatile live snapshot. The event is the wire-visible evidence and neither form survives restart. A PCI reconnect or transport loss invalidates an in-flight snapshot before commit, returns 408, and emits no sync-ok |
