...
one valid known IDENTIFY4 serial. Live direct-network evidence includes healthy,
uniquely identified units in both states; the complete IDENTIFY4 reply window is
the independent uniqueness guard. State three is a native error flag and does
not receive these source-address-only reads, nor do zero-serial or multi-serial
addresses. This
