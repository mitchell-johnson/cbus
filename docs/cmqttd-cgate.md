...
| NET SYNC and cached unit getters | Configured interface routing hint (physically revalidated) or BASIC discovery, complete installation MMI, then confirmed IDENTIFY1/2 probes and bounded IDENTIFY4 collection for every present address; routed and local bare-CAL replies are correlated, silent legacy/error addresses remain present with unknown identity fields, the live cache is replaced atomically, and native getters expose it. A unit classified KEYGL5 by both the configured database and fresh IDENTIFY1 also receives the native parameter-`0xFA`, length-44 WidgetGroups recall; `GET ... WidgetGroups` serves its opaque bytes as decimal CSV in a `300` envelope without new bus I/O. An address with multiple distinct serials emits `#e# net {network} sync duplicate {address} {serial...}` (sorted); scalar `SerialNumber` stays `""` while the sorted set of distinct serials is retained in the volatile live snapshot. The event is the wire-visible evidence and neither form survives restart |
...
The KEYGL5 WidgetGroups request is exactly one checksummed point-to-point CAL
`RECALL 0xFA 44`. Replies must match the unit source and parameter and assemble
to exactly 44 bytes. cmqttd preserves native CBusEdlt sync scope: failure of
this optional property does not discard otherwise valid identity results or
turn `NET SYNC` into an error. It removes any older volatile WidgetGroups value
so `GET` cannot serve stale bytes. A timeout or lost response still faults the
programming lane until reconnect because a late untagged CAL reply cannot be
safely attributed, and the request is never replayed. `CMQTT CAPABILITIES`
reports this bounded support as `"edlt_widget_groups": true`. The 44 bytes are
reported without interpretation; this does not query dynamic-label cache
contents, rendering, labels or persistence. Parameter `0xFB` length 9 remains
the distinct extended-firmware read documented by native CBusEdlt bytecode.
