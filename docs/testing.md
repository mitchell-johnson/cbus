...
`cgate-mock` tests exercise tagged framing, multiline replies, shared state, per-session project selection, event filtering and fanout, here-documents, command inventory reachability, and programming access. The hardware-service tests separately pin bounded `DBSETXML` document framing; durable project archive/restore/rename/copy/delete rollback; selected-project `DBTAGLIST`, scalar `DBSET`, and secondary-project network readdressing across restart; read-only repository listing; and the absence of PCI traffic for these local administrative commands. `rust/testdata/fixtures/native_cgate_legacy_database.json` records the exact legacy-database oracle and the six operations deliberately retained at 502. `cmqttd/tests/system_cgate_project_copy_delete.rs` is the dedicated real-daemon copy/delete and MQTT-continuity regression.
