...
`cmqttd` connects one C-Bus PCI/CNI endpoint to one MQTT broker and can also serve C-Gate clients through `--cgate-bind`. Both interfaces share one transport. Incoming C-Bus frames become typed protocol values, state publications, and Home Assistant discovery messages. Valid MQTT commands and supported C-Gate lighting, Trigger, Enable, clock, install-MMI discovery and read-only physical PP LOAD commands become C-Bus traffic. A Toolkit `.cbz` or bare XML project supplies human-readable network, application, group, and unit metadata; the embedded C-Gate service imports it into a persistent database. Physical PP LOAD also requires private decoded unit specifications supplied with `--cgate-unitspec`.
