...
14. For retained SceneManager edits, use `--project-xml ... --unit ...` offline or `--auto-metadata --exclusive-project` on the database command when existing native metadata is authoritative. This path creates no application, group or level. Keep the exact source-network lock and every project network closed/idle. Inspect consumed trigger/action rows and reject image-dependent facts rather than inferring them. Native apply has one PP SAVE boundary: do not retry an uncertain save reply. Missing level creation and complete SceneManager control binding remain unsupported.
