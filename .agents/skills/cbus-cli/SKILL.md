...
19. cmqttd's PP catalogue, lock/session inventory, raw-memory diagnostics and PROGRAMMER queue administration are local compatibility operations. Supply private decoded specs only through `--cgate-unitspec`; PP file arguments must be bare names under that directory. Treat SET_RAW_DATA and LOAD_FROM_FILE as staged session edits, not device writes. PROGRAMMER queues are runtime-only. `PROGRAMMER TRIGGER ... START` and `PP WRITE_PATCH` remain explicit 502 physical-execution boundaries; never report queue or patch completion, retry against a device, or substitute a simulated success. Read [references/cgate.md](references/cgate.md) for syntax, authentication, status envelopes and retained evidence.
