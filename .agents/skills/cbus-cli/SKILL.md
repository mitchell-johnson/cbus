...
14. For retained SceneManager edits, use `--project-xml ... --unit ...` offline or `--auto-metadata --exclusive-project` on the database command when native metadata is authoritative. Inspect `planned_creations`: exact missing actions reached by retained getters/setters are created under their existing Trigger Control group as `Action Selector N`, with Address=Value=N and four blank variants. This path creates no application or trigger group and never infers an image-dependent label. Keep the exact source-network lock and every project network closed/idle. On apply, prefer a reviewed `--backup-project`; DBADDSAFE/DBSETSAFE, PP SAVE and PROJECT SAVE are separate. Automatic rollback stops before the first applicable PP or target-project save, and no uncertain save may be retried. Complete SceneManager control binding remains unsupported.
