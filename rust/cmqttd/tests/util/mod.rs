...
/// A /set command drains behind up to 384 throttled status requests
/// (0.2 s each) — that timing is part of Python parity.
pub const COMMAND_DRAIN: Duration = Duration::from_secs(150);
...
pub async fn start_with(opts: Options) -> System {
    ...
    let pci = FakePci::start(opts.withhold_status_conf).await;
...
/// (Status requests may interleave: the MQTT session comes up while the
/// init sequence is still being paced out at 0.1 s per write.)
...
/// Block until cmqttd finished its PCI init, subscribed the wildcard and
/// started streaming status requests — the common "fully up" baseline.
pub async fn wait_started(sys: &System) {
    ...
    require(STARTUP, "first status request", || {
        sys.pci.payloads().iter().any(|p| p.starts_with("05FF0073"))
    })
    .await;
}
