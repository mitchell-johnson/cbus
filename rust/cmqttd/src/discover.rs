//! mDNS discovery of C-Bus bridges: browse `_cbus._tcp.local.` and connect
...
/// `(host, port)`. This returns as soon as one device resolves.
    ...
    while found.is_none() {
        ...
        match receiver.recv_timeout(deadline - now) {
            Ok(mdns_sd::ServiceEvent::ServiceResolved(info)) => {
                ...
                // Prefer IPv4 for C-Bus bridge compatibility.
