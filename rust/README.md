...
| `cbus-cgate`     | lib  | in-memory C-Gate 3.4 command model: tagged framing (incl. multi-status envelopes), project/network lifecycle (`DBCREATENET`, `NET OPEN/CLOSE/SYNC/SYNCNEW/PINGU/CHECKUNIT/UNRAVEL/CLOCKS/LIST/RENAME`, `TREEXML` variants, `CALCULATOR TEST`), database verbs (`DBADDSAFE` with 301/342 Level-OID flow, `DBCOPYSAFE`, `DBRENAMENETSAFE`, `DBDELETE`, `DBVALIDATE`, `DBSETSAFE`, `DBSETXML`/`CGL IMPORT` here-docs), `DBGET`/`DBGETXML` (342 field rows, 347 snippets, Network XML), `GET` cached properties in native 300 shape (levels, serials fields, 21-field runtime, `Units`), serials multiplicity (`120` rows), PP programming sessions (LOCK/START/END/NEW/LOAD/GET/SET/SAVE/QUICKGET/INFO with mK dequoting, catalogue-backed INFO/defaults via unitspec reader), lighting/trigger/enable/label commands, `EVENT`/`GETSTATE` subscription, role-based access levels |
| `cgate-mock`     | bin  | loopback TCP test double serving `cbus-cgate` (`cgate-mock --bind 127.0.0.1:0 [--unitspec DIR]`); the Python `CGateClient` interop suite drives it directly |
