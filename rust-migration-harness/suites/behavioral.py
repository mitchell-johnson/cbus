...
the PCI init sequence, HA discovery publishes, status-request traffic,
retransmission on missing confirmation, C-Bus event -> MQTT state flow,
level report -> MQTT state flow, clock request handling, and
MQTT command -> C-Bus frame flow.
...
NOTE ON TIMING: cmqttd throttles C-Bus commands at one per 0.2s and
enqueues 384 status requests at startup (48 lighting apps x 8 blocks).
Injected MQTT commands drain *behind* that queue, so the two
`mqtt-cmd-*` assertions legitimately take up to ~2.5 minutes. This is
parity-faithful behaviour, not a harness bug. `--skip-slow` skips them.
"""
import argparse
import asyncio
import json
...
from wire import CONFIRMATION_CODES  # noqa: E402
...
async def run(impl: str, rust_bin: str, skip_slow: bool) -> int:
    res = Results()
    fake = FakePCI(withhold_status_request_conf=True)
    broker = MiniBroker()
    await fake.start()
    await broker.start()
    ...
    try:
        ...
        def dm_sequence():
            ...
            if payloads[:4] != want:
                return None
            confs = [g[1] for g in got[:4]]
            if any(c is None or c[0] not in CONFIRMATION_CODES
                   for c in confs):
                return None
            if len(set(confs)) != 4:
                return None
        ...
        res.record('pci-init-dm-sequence', bool(ok),
                   f'payloads seen: {fake.payloads()[:12]}')
        ...
        ok = await wait_for(
            lambda: broker.has_subscription('homeassistant/light/#'), 15)
        ...
        # ---- inject the MQTT commands now: they drain behind the -------
        # ---- 384-entry status request throttle queue --------------------
        ...
        if not skip_slow:
            ...
            print('  (mqtt commands injected; frames will drain behind the '
                  'status-request throttle queue)', flush=True)

        # ---- status request traffic --------------------------------------
        valid_payloads = set(EXPECT['status_requests'].values())

        def status_seen():
            seen = [p for p in fake.payloads() if p.startswith('05FF0073')]
            if len(seen) < 10:
                return None
            bad = [p for p in seen if p not in valid_payloads]
            return (True if not bad else None)
        ok = await wait_for(status_seen, 40)
        seen = [p for p in fake.payloads() if p.startswith('05FF0073')]
        bad = [p for p in seen if p not in valid_payloads]
        res.record('status-requests-valid', bool(ok),
                   f'{len(seen)} seen, malformed: {bad[:3]}')
        ...
        # ---- MQTT command -> C-Bus frame (slow: throttle queue) ----------
        if skip_slow:
