"""Exact KEYGL5 MultiLevel thresholds and shared status text in database PP."""
...
class MultiLevelWidgetPlan:
    ...
    def as_dict(self):
        return {'format': 'cbus-edlt-multilevel-plan-v1', 'unit_type': 'KEYGL5',
                ...
                'levels': self.levels, 'low_threshold': self.record[7], 'high_threshold': self.record[8],
...
class EdltMultiLevelWidget:
    ...
    def plan(self, current, *, page, position, group, application='primary', levels=None,
        ...
        options = dict(page=page, position=position, group=group, application=application,
                       levels=levels, low_threshold=low_threshold, high_threshold=high_threshold,
        ...
        if app not in APPLICATIONS:
            raise EdltError('MultiLevel requires an assigned application in 48..127 or 136')
        ...
        if record[0] != 16:
            # Original MultiLevelData.SetToDefault follows the AppGroup base:
            # blank label255, static Off status, icons134/134, and four calls.
            # The low/medium/high references remain used when their UI is hidden.
            record[0:4] = (16, 5, 134, 134)
        ...
        if status_forced and (off_text is not None or off_index is not None):
            raise EdltError('Normalize existing nonstatic MultiLevel status first, then edit its Off reference')
        if levels is not None:
            record[7:9] = {1: (0, 0), 2: (127, 127), 3: (84, 170)}[levels]
        else:
            low, high = record[7:9]
            levels = 1 if low == high == 0 else 2 if 1 <= low == high <= 254 else 3 if 1 <= low < high <= 254 else None
            if levels is None:
                raise EdltError('Invalid existing MultiLevel thresholds; supply levels to reset them')
        if levels == 1:
            if low_threshold is not None or high_threshold is not None:
                raise EdltError('One-level MultiLevel does not use thresholds')
        elif levels == 2:
            if high_threshold is not None:
                raise EdltError('Two-level MultiLevel uses low_threshold for both stored thresholds')
            ...
            if not 1 <= record[7] < record[8] <= 254:
                raise EdltError('Three-level MultiLevel requires 1 <= low_threshold < high_threshold <= 254')
        for name, minimum in (('low', 2), ('medium', 3)):
            if levels < minimum and any(value is not None for value in requested_status[name]):
                raise EdltError(f'{name} status is hidden for this MultiLevel level count; existing reference is preserved')
        ...
        return MultiLevelWidgetPlan(page, position, widget, page_mode, group, app, levels, restore,
    ...
    def apply(self, session, plan):
        if not isinstance(plan, MultiLevelWidgetPlan) or not isinstance(plan.options, Mapping):
            raise EdltError('Use a MultiLevel widget plan returned by EdltMultiLevelWidget.plan')
        _int(plan.levels, 'Plan levels', 1, 3)
        ...
        try:
            ...
            raise EdltError('Invalid MultiLevel plan settings') from error
        if canonical != plan:
            raise EdltError('Plan differs from its validated MultiLevel settings')
        ...
        if self.snapshot(session.values()) != dict(plan.expected):
            raise EdltError('PP values changed since the MultiLevel plan was made')
        attempted = []
        try:
            ...
            if self.snapshot(session.values()) != expected:
                raise EdltError('Native PP readback differs from the MultiLevel plan')
