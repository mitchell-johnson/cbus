"""Exact KEYGL5 Timer Toggle/Retrigger database PP configuration.

Original TimerData defaults allocate Fan text and normalize target zero to one.
Duration, expiry and ramp fields follow the exact model and WinForms bounds.
"""
...
from .edlt import (EdltLighting, EdltError, EdltApplyError, StaticTextAllocation,
                   LABEL_TYPES, RAMP_SECONDS, _field, _int, _render)

# Exact subclasses of StatusLabelAppGroupData in the original model.
_APP_GROUP_TYPES = frozenset((2, 3, 4, 5, 14, 15, 16))
TIMER_STATUS_TYPES = MappingProxyType({'blank': 0, 'timer': 4, 'static': 5, 'dynamic-text': 6, 'dynamic-icon': 7})


@dataclass(frozen=True)
class TimerWidgetPlan:
    page: int
    position: int
    widget: int
    page_mode: str
    ...
    def as_dict(self):
        return {'format': 'cbus-edlt-timer-plan-v1', 'unit_type': 'KEYGL5',
                'catalog_number': '5055EDL', 'firmware': '5.5.00', 'mode': 'toggle-retrigger',
                ...
                'duration_seconds': self.record[12] + 256 * self.record[13], 'target_level': self.record[9],
                'expiry_level': self.record[14] + 256 * self.record[15], 'ramp_seconds': RAMP_SECONDS[self.record[16]],
...
class EdltTimerWidget:
    ...
    def plan(self, current, *, page, position, group, application='primary',
             duration_seconds=None, target_level=None, expiry_level=None, ramp_seconds=None,
             page_mode=None, label_type=None, label_index=None, label_text=None,
        ...
        options = dict(page=page, position=position, group=group, application=application,
                       duration_seconds=duration_seconds, target_level=target_level,
                       expiry_level=expiry_level, ramp_seconds=ramp_seconds, page_mode=page_mode,
        ...
        _int(group, 'Timer group', 0, 254)
        if application not in ('primary', 'secondary'):
            raise EdltError('application must be primary or secondary')
        for value, name, minimum, maximum in ((duration_seconds, 'Timer duration seconds', 0, 64800),
                (target_level, 'Timer target level', 1, 255), (expiry_level, 'Timer expiry level', 0, 255),
                (ramp_seconds, 'Timer ramp seconds', 0, 1020)):
            if value is not None:
                _int(value, name, minimum, maximum)
        if ramp_seconds is not None and ramp_seconds not in RAMP_SECONDS:
            raise EdltError('Timer ramp seconds must use a supported ramp time')
        ...
        if not 48 <= app <= 95:
            raise EdltError('Timer requires an assigned Lighting application in 48..95')
        ...
        if record[0] not in (0, 5, 255):
            raise EdltError('Selected widget is another type; replacing it is outside this workflow')
        fresh = record[0] != 5
        default_allocation = None
        if fresh:
            # TimerData.SetToDefault uses Timer status and static Fan label.
            # Both overridden index properties reset on those category edits.
            # Bytes4/5/10/11/19..31 are opaque and remain untouched.
            record[0:4] = (5, 0x34, 14, 13)
            record[6:10] = (255, 35, 34, 255)
            record[12:19] = (60, 0, 0, 0, 15, 0, 0)
            view = self.common._place_record(updates, widget, record)
            default_allocation = self.common.allocate_static_text(view, 'Fan')
            updates.update(default_allocation.changes)
            record[17] = default_allocation.index
        ...
        record[6:9] = (group, 35, 34)
        if duration_seconds is not None:
            record[12:14] = duration_seconds.to_bytes(2, 'little')
        if record[12] + 256 * record[13] > 64800:
            raise EdltError('Invalid existing Timer duration; supply duration_seconds in 0..64800')
        if target_level is not None:
            record[9] = target_level
        if record[9] == 0:
            record[9] = 1  # Exact TimerData.SetForcedValues, selected widget only.
        if expiry_level is not None:
            record[14:16] = expiry_level.to_bytes(2, 'little')
        if record[15] != 0:
            raise EdltError('Invalid existing Timer expiry; supply expiry_level in 0..255')
        if ramp_seconds is not None:
            record[16] = RAMP_SECONDS.index(ramp_seconds)
        if record[16] >= len(RAMP_SECONDS):
            raise EdltError('Invalid existing Timer ramp index; supply ramp_seconds')
        allocations = [None, None]
        for entry, (option, index, text, codes, shift, mask, slot, static_code, dynamic_codes) in enumerate((
                (label_type, label_index, label_text, LABEL_TYPES, 4, 0x70, 17, 3, (1, 2)),
                (status_type, status_index, status_text, TIMER_STATUS_TYPES, 0, 0x0f, 18, 5, (6, 7)))):
            ...
            if (option is None and previous_code in dynamic_codes
                ...
                # Timer's index setters always refresh network metadata,
        ...
        return TimerWidgetPlan(page, position, widget, page_mode, group, app, restore,
                                 restore_source, original, changes, bytes(record), default_allocation, *allocations, options)

    def apply(self, session, plan):
        if not isinstance(plan, TimerWidgetPlan) or not isinstance(plan.options, Mapping):
            raise EdltError('Use a Timer widget plan returned by EdltTimerWidget.plan')
        ...
        try:
            ...
            raise EdltError('Invalid Timer plan settings') from error
        if canonical != plan:
            raise EdltError('Plan differs from its validated Timer settings')
        ...
        if self.snapshot(session.values()) != dict(plan.expected):
            raise EdltError('PP values changed since the Timer plan was made')
        attempted = []
        try:
            ...
            if self.snapshot(session.values()) != expected:
                raise EdltError('Native PP readback differs from the Timer plan')
