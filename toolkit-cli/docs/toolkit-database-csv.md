...
The multi-unit extension contributes 17 focused API/CLI tests using committed fixtures and ephemeral loopback C-Gate peers. They check explicit and document order, a single snapshot parse/request, mixed admitted relay/key profiles, legacy OID-less identities, cross-network selection, empty networks, duplicate/missing selections, late unsupported profiles, read-only missing-Area rejection and exclusive output. The address-only three-unit ordering regression reuses the committed original `manager-order` serializer vector with admitted native relay records; it does not admit that vector's KEYGL5 native profile. These tests verify composition of the existing bounded projector and serializer. They do not provide new original Toolkit execution, Windows acceptance, physical acceptance or proof of full Document Database parity. Original manager enumeration and the remaining unit profiles remain outstanding; the feature ledger remains in progress.
