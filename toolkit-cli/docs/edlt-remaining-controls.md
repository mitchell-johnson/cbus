...
action `DynamicAll` rows from one exact project snapshot. It does not create
missing action levels or establish the remaining SceneManager control binding.
...
| **Automatic retained SceneManager metadata** | Existing native application/group lists, consumed trigger-action addresses and safe default-language text rows are resolved automatically; [contract](edlt-scene-metadata.md) | Exact project/PP stale checks, closed-project/exclusive-owner guards, connected PP rollback, single-save uncertainty and non-PP metadata preservation are explicit. No database metadata is created. Missing level creation, project/DLTP images, full SceneManager control binding and physical behavior remain open. |
