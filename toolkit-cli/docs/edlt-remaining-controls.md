...
action `DynamicAll` rows from one exact project snapshot. It also creates exact
missing action levels reached by the retained getter/setter, behind backup,
stale, rollback and separate PP/project-save evidence. It does not create
missing trigger groups or establish the remaining SceneManager control binding.
...
| **Automatic retained SceneManager metadata** | Existing native application/group lists, consumed trigger-action addresses and safe default-language text rows are resolved automatically; exact missing action levels are projected/created; [contract](edlt-scene-metadata.md) | `Action Selector N`, exact Address/Value and four blank variants are source-backed. Exact pre/post-backup stale checks, closed-project/exclusive-owner guards, pre-PP-save rollback, separate PP/PROJECT SAVE uncertainty and reload preservation are explicit. Missing trigger-group creation, the interactive blank add dialog, project/DLTP images, full SceneManager control binding and physical behavior remain open. |
