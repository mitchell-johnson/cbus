...
The two-serial extension identified by this audit has since been implemented as an independent [serial-keyed duplicate fixture](serial-address-fixture.md). It retains separate per-serial memory and address state, correlated identity replies, selected-serial movement and persisted before/after observations. [Native duplicate discovery](duplicate-discovery.md) and the separate [selected-serial plan/apply/verify coordinator](pci-selected-serial.md) have scoped acceptance for exactly two known identities at255 and an empty explicit destination. The coordinator does not reuse raw UNRAVEL's broader fallback behavior. General duplicates, occupied destinations, cycles, local PCI/bridge relocation and physical firmware persistence remain open.
