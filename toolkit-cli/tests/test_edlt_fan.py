"""Independent original FanControllerData vectors and native PP persistence."""
...
BOUND = '0435040300002A54AA3B3F3E3D3C000000000000000000000000000000000000'
ONE = '04B5040300002A00003B3F3E3D3C000000000000000000000000000000000000'
TWO = '04B5040300002A7F7F3B3F3E3D3C000000000000000000000000000000000000'
TWO_MAX = '04B5040300002AFEFE3B3F3E3D3C000000000000000000000000000000000000'
THREE = '04B5040300002A54AA3B3F3E3D3C000000000000000000000000000000000000'
THREE_MIN = '04B5040300002A01023B3F3E3D3C000000000000000000000000000000000000'
THREE_MAX = '04B5040300002AFDFE3B3F3E3D3C000000000000000000000000000000000000'
STATIC = '04B5040300002A54AA3A3B3F3E3D000000000000000000000000000000000000'
ICONS = '04A5040300002A54AA013B3F3E3D000000000000000000000000000000000000'
TEXT = '0495040300002A54AA023B3F3E3D000000000000000000000000000000000000'
SAME_INDEX = '04A5040300002A54AA023B3F3E3D000000000000000000000000000000000000'
FORCED = '04A5040300002A54AA02003F3E3D000000000000000000000000000000000000'
OPAQUE = '043504030405FF54AA3B3F3E3D3C0E0F101112131415161718191A1B1C1D1E1F'
NATIVE = {
 'default': '0435040300002A54AA020C0D0E0F000000000000000000000000000000000000',
 'one': '04B5040300002A0000020C0D0E0F000000000000000000000000000000000000',
 'two': '04B5040300002AFEFE020C0D0E0F000000000000000000000000000000000000',
 'three': '04B5040300002AFDFE020C0D0E0F000000000000000000000000000000000000',
 'custom': '04B5040300002AFDFE3F3E3D3C3B000000000000000000000000000000000000',
 'explicit-indexes': '04B5040300002AFDFE3F32333435000000000000000000000000000000000000',
 'first-text-match': '04B5040300002AFDFE3F0C0D0E0F000000000000000000000000000000000000',
 'index-boundaries': '04B5040300002AFDFE3F003F003F000000000000000000000000000000000000',
 'dynamic-text': '0495040300002AFDFE023E3D3C3B000000000000000000000000000000000000',
 'same-index': '04A5040300002AFDFE023E3D3C3B000000000000000000000000000000000000',
 'force-static': '04A5040300002AFDFE02003D3C3B000000000000000000000000000000000000',
}
...
class FanWidgetTests(unittest.TestCase):
    def setUp(self):
        self.spec = fixture(); self.editor = EdltFanWidget(self.spec); self.session = Session(self.spec)
    ...
    def test_literal_default_speed_thresholds_and_explicit_reset(self):
        ...
        self.assertEqual(plan.speeds, 3); self.assertFalse(self.plan(after(plan)).changes)
        for options, literal in ((dict(speeds=1), ONE), (dict(speeds=2), TWO),
                (dict(low_threshold=254), TWO_MAX), (dict(speeds=3), THREE),
                (dict(low_threshold=1, high_threshold=2), THREE_MIN),
                (dict(low_threshold=253, high_threshold=254), THREE_MAX), (dict(speeds=3), THREE)):
        ...
        with self.assertRaisesRegex(EdltError, 'existing Fan thresholds'): self.plan(invalid)
        self.assertEqual(self.plan(invalid, speeds=2).record[7:9], b'\x7f\x7f')

    def test_original_nine_default_allocations_reuse_and_sequential_dedup(self):
        ...
        self.assertEqual([(k, a.index) for k, a in plan.default_allocations],
            [('off',63),('low',62),('medium',61),('high',60),('label',59),('off',63),('low',62),('medium',61),('high',60)])
        self.assertTrue(all(a.reused for _,a in plan.default_allocations[5:]))
        self.assertEqual(plan.static_allocation.index, 58)
        self.assertEqual([a.index for a in plan.status_allocations.values()], [59,63,62,61])
        ...
        blank = self.plan(label_type='blank', speeds=1)
        self.assertEqual(len(blank.default_allocations), 9)
    ...
    def test_explicit_duplicate_indexes_and_hidden_reference_retention(self):
        ...
        one = self.plan(after(plan), speeds=1)
        ...
        two = self.plan(after(one), speeds=2, low_index=63)
        self.assertEqual(two.record[11:14], bytes((63,52,53)))
        bounds = self.plan(after(two), speeds=3, off_index=0, low_index=63, medium_index=0, high_index=63)
    ...
    def test_full_table_and_profile_layout_range_guards(self):
        for options in ({'speeds':0},{'speeds':4},{'speeds':True},{'low_threshold':0},{'high_threshold':255},
                {'low_threshold':True},{'low_threshold':171},{'high_threshold':84},{'speeds':1,'low_threshold':1},
                {'speeds':2,'high_threshold':1},{'off_text':'X','off_index':1},{'low_index':True},{'medium_index':64},
        ...
        for settings in ({'catalog_number':'5055EDLB'},{'firmware':'6.0.00'}):
            with self.assertRaises(EdltError):EdltFanWidget(self.spec,**settings)
    ...
    def test_stale_identity_canonical_guards_and_unsaved_apply(self):
        plan=self.plan()
        for forged in (None,replace(plan,widget=True),replace(plan,speeds=False),replace(plan,record=b'bad'),
...
@unittest.skipUnless(all(os.environ.get(key) for key in ('CBUS_CGATE_TEST_HOST','CBUS_UNITSPEC_DIR','CBUS_TOOLKIT_EXE')),
                     'Set C-Gate host, specifications and Toolkit EXE for native Fan widget acceptance')
class NativeFanWidgetTests(unittest.TestCase):
    def test_original_model_native_raw_shared_refs_crc_and_save_reload(self):
        ...
        editor=EdltFanWidget(spec);project='EFN'+uuid4().hex[:5].upper();network=f'//{project}/254';unit=network+'/p/20'
        report={'project':project,'scope':'5055EDL5.5 Fan1/2/3 speeds database PP; no physical device','passed':False}
        backend = selected_backend(); report['original_backend'] = backend
        with tempfile.TemporaryDirectory() as folder:
            for name in ('NativeEdltFanProbe.cs','NativeEdltProbe.cs'):
            ...
            oracle = OriginalModelOracle(root / 'research/NativeEdltFanProbe.cs', app, backend='windows') if backend == 'windows' else None
            ...
            if oracle is not None:
                ...
                model=mono('mcs -r:/input/CBusLogicModel.dll -r:System.Windows.Forms -r:System.Drawing NativeEdltFanProbe.cs && MONO_PATH=/input mono NativeEdltFanProbe.exe')
            ...
            with CGateClient(os.environ['CBUS_CGATE_TEST_HOST'],int(os.environ.get('CBUS_CGATE_TEST_PORT','20023')),timeout=30) as client:
                ...
                try:
                    for command in ('PROJECT USE '+project,f'DBSET //{project}/Project/Description cbus-toolkit-isolated-edlt-fan-v1',
                    ...
                    with programmer.load(network,'/db'+unit) as session:
                        ...
                        sequence=(('one',dict(speeds=1)),('two',dict(speeds=2,low_threshold=254)),
                            ('three',dict(speeds=3,low_threshold=253,high_threshold=254)),('custom',CUSTOM),
                    ...
                    with programmer.load(network,'/db'+unit) as session:
                        ...
                        with self.assertRaises(EdltError):editor.configure(session,page=4,position=4,page_mode='multiple',group=43,speeds=1,low_index=1)
                        self.assertEqual(editor.snapshot(session.values()),final)
                    ...
                    if os.environ.get('CBUS_EDLT_FAN_REPORT'):Path(os.environ['CBUS_EDLT_FAN_REPORT']).write_text(json.dumps(report,indent=2)+'\n')
