// Execute unchanged original Measurement icon setter and full PP save/CRC hooks.
using System;
using System.Linq;
using System.IO;
...
class NativeEdltMeasurementIconProbe {
 ...
 static void Fixture(string[] args) {
  ...
  if(args[13]!="-")d.BigIconIndex=int.Parse(args[13]);
 ...
 static void Main(string[] args) {
  PPAttribute.bInitialiseMode=true;if(args.Length==14){Fixture(args);return;}
  foreach(int n in new[]{1,5,6,10,21}) {
   var unit=Unit();var widget=unit.Widgets[n-1];widget.WidgetType=12;var data=(MeasurementData)widget.WidgetData;
   Dump("default-"+n,widget);
   for(int icon=0;icon<256;icon++) {
    data.BigIconIndex=icon;
    if(data.BigIconIndex!=icon)throw new Exception("Icon getter/setter mismatch");
    data.SetForcedValues();Dump("icon-"+n+"-"+icon,widget);
   }
   data.BigIconIndex=200;unit.GetPPAttribute("UseBigIcon").ValueAsInt=0;
   widget.WidgetType=12;Save(unit);Dump("hidden-same-type-"+n,widget);
  }
 }
}
