"""Owned closed C-Gate Enable Control level backend probe, without device I/O."""
...
def levels():
    ...
    for level in node.findall('Level'):
        value = int(level.get('Value'))
        assert value not in result
        result[value] = {'tag': level.findtext('TagName'), 'address': level.get('Address'),
