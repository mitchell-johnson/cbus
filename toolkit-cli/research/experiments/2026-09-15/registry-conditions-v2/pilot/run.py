...
def validate_original_provenance(records,contract,compiled_sha256):
    ...
    provider=before.get('registry_provider')
    ...
    for stage in (before,records[-2]):
        ...
        required={'SE.DAD.SESU.Common','Microsoft.Win32.Registry','mscorlib',contract['prefix']+'-probe'}
...
def main(release):
    if release!='root-reviewed-registry-twelve-v2':raise ValueError('Exact parent-reviewed release required')
    ...
    output=HERE/'pilot-v2';output.mkdir(exist_ok=False)
    ...
    def stage(name,script):
        job_id='job-registry-'+name+'-d0fe9896d3764e53'
...
if __name__=='__main__':
    ...
    print(json.dumps({'passed':result['passed'],'root_name':result['root_name'],'report':str(HERE/'pilot-v2/report.json')}))
