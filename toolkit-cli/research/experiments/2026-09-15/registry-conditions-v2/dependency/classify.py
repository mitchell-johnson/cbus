...
def main():
    ...
    (HERE/'classification-input-pins.json').write_text(json.dumps(inputs,indent=2)+'\n')
    ...
    for target,path,option in [('forwarder',forwarder,'--assembly'),('forwarder',forwarder,'--assemblyref'),
        ...
        result=subprocess.CompletedProcess(command,0,(HERE/(target+'-'+option[2:]+'.stdout')).read_bytes(),(HERE/(target+'-'+option[2:]+'.stderr')).read_bytes())
        name=target+'-'+option[2:]
        # Reuse the six metadata-only outputs; no disassembler is invoked by this classification.
    ...
    if 'Method Table (1..63)' not in methods or re.search(r'(?m)^\d+: Microsoft\.Win32\.Registry(?: |$)',types) or 'GetValue (' in methods:
        raise RuntimeError('Registry implementation unexpectedly present in forwarding facade')
    ...
    report={'format':'registry-staged-forwarder-inspection-v1','original_methods_executed':False,'guest_access':False,'reuses_existing_metadata_outputs':True,'previous_local_classification_error':'Initial static assertion incorrectly expected zero total methods; actual facade has 63 resource/attribute helpers and no Registry implementation. Original failure and metadata outputs are unchanged.',
            ...
            'forwarder_total_method_count':63,'registry_implementation_method_count':0,'helper_and_attribute_methods_present':True,'implementation_assembly_reference':'mscorlib, Version=4.0.0.0, PublicKeyToken=b77a5c561934e089',
