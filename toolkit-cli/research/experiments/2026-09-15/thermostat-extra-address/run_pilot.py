...
TOKEN='root-reviewed-thermostat-levels-extra-address-v1'
...
def associate_child(result, *, expected, cases, partial=False):
 if type(result) is not dict or result.get('format')!='thermostat-levels-original-extra-address-v1':raise ValueError('Child report format')
...
def main():
 ...
 try:
  ...
  result=json.loads(read(out/'original/report.json',32*1024*1024));assert result['passed'] and len(result['results'])==2
  ...
  for case,row in zip(cases,result['results']):
   levels=row['state']['levels'];assert [v['address'] for v in levels]==case['expected']['final_level_order']
   assert levels[:len(case['existing'])]==row['initial_state']['levels']
