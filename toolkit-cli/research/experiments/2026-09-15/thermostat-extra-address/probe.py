...
def validate(case):
 ...
 if case['kind']=='zones':
  ...
  require(type(case['existing']) is list and len(case['existing'])<=34,'Existing bound')
  ids=[]
  for level in case['existing']:
   require(type(level) is dict and set(level)=={'address','value','tag'},'Existing schema')
   require(type(level['address']) is int and 0<=level['address']<=255 and type(level['value']) is int and 0<=level['value']<=255,'Existing numbers')
...
def main():
 require(len(sys.argv)==5 and sys.argv[1]=='root-reviewed-thermostat-levels-extra-address-v1','Reviewed execution release required')
 ...
 require(type(cases) is list and len(cases)==2,'Exact extension2')
 for case in cases:validate(case)
 require(len({c['id'] for c in cases})==2,'Unique IDs')
 image=Image(exe);out.mkdir(exist_ok=False)
 report={'format':'thermostat-levels-original-extra-address-v1','passed':False,'python':sys.version,'cases_sha256':digest(raw),'probe_sha256':digest(bounded_read(Path(__file__),1024*1024)),'exe_sha256':digest(image.raw),'spans':image.spans,'results':[],
