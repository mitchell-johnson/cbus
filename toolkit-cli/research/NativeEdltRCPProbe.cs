...
class NativeEdltRCPProbe {
 ...
 static RCPData Rcp(EDLTUnit u) {
  var w=new EDLTWidget{Unit=u,Attributes=u.PPAttributes,WidgetNumber=6};var d=new RCPData(w,"Widget6",u.PPAttributes);w.WidgetData=d;u.Widgets.Add(w);return d;
 }
 static void Dump(string label,RCPData d) {
  var bytes=new byte[32];bytes[0]=15;for(int i=1;i<32;i++)bytes[i]=(byte)d.WidgetByte(i).ValueAsInt;
 ...
 static void Main() {
  ...
  Console.WriteLine("colours:"+string.Join("|",constants.KeyColours.Select(x=>x.Name+"="+x.ValueAsInt)));
  Console.WriteLine("modes:"+string.Join("|",constants.DualKeyMacroFunctionsRCP.Select(x=>x.Name+"="+x.ValueAsInt)));
  Console.WriteLine("statuses:"+string.Join("|",constants.FunctionStatusTypesRCP.Select(x=>x.Name+"="+x.ValueAsInt)));
  var u=Unit();var d=Rcp(u);d.SetToDefault();Dump("default",d);Console.WriteLine("default-text-count:"+u.StaticLabels.Count(x=>x.Name!=""));
  d.GroupAddress=42;Dump("bound-default",d);d.ApplicationVariant=1;d.OffColour=2;d.OnColour=3;d.KeyMacrofunction=255;Dump("unused-colours",d);
  d.LabelDisplayType=3;d.LabelValueText="Courtesy";d.StatusDisplayType=5;d.StatusValueText="Available";Dump("custom-static",d);
  d.KeyMacrofunction=25;Dump("bell-retains-labels",d);
  for(int i=0;i<9;i++){d.OffColour=i;d.OnColour=8-i;Dump("colour-"+i,d);}
  d.OffColour=2;d.OnColour=3;d.LabelDisplayType=10;d.LabelValueIndex=1;d.StatusDisplayType=10;d.StatusValueIndex=3;Dump("dynamic-icons",d);
  d.LabelValueIndex=2;d.StatusValueIndex=0;Dump("dynamic-text",d);
  d.SelectedGroup.DynamicAll[2].Image=Bare<Bitmap>();d.SelectedGroup.DynamicAll[0].Image=Bare<Bitmap>();d.LabelValueIndex=2;d.StatusValueIndex=0;Dump("same-index-refresh",d);
  ...
  d.GroupAddress=43;Dump("group-refresh",d);Console.WriteLine("restore-display:"+d.GetRestoreLevelText());
  foreach(int app in new[]{127,136}){u.GetPPAttribute("SecondaryApplication").ValueAsInt=app;d.ApplicationVariant=1;Dump("application-"+app,d);Console.WriteLine("selected-application:"+d.GetSelectedApplication().AddressAsInt);}
  d.LabelDisplayType=0;d.StatusDisplayType=0;Dump("blank-types",d);
  d.StatusIconIndex=137;Dump("single-icon-both-bytes",d);
  d.SetForcedValues();Dump("no-forcing",d);
  foreach(bool zero in new[]{false,true}){var other=Rcp(Unit(true,zero));other.SetToDefault();Dump(zero?"opaque-blank-default":"opaque-default",other);}
 }
}
