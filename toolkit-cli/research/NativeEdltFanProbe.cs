// Calls original FanControllerData constructors/properties on explicit PP scaffolds.
using System;
using System.Linq;
...
class NativeEdltFanProbe {
 ...
 static FanControllerData Fan(EDLTUnit u) {
  var w=new EDLTWidget{Unit=u,Attributes=u.PPAttributes,WidgetNumber=6};var d=new FanControllerData(w,"Widget6",u.PPAttributes);w.WidgetData=d;u.Widgets.Add(w);return d;
 }
 static void Dump(string label,FanControllerData d) {
  var bytes=new byte[32];bytes[0]=4;for(int i=1;i<32;i++)bytes[i]=(byte)d.WidgetByte(i).ValueAsInt;
 ...
 static void Main() {
  ...
  var u=Unit();var d=Fan(u);d.SetToDefault();Dump("default",d);Texts(u);
  ...
  d.LabelValueText="Ceiling";d.StatusValueText="Stopped";d.LowStatusText="Slow";d.MedStatusText="Normal";d.HighStatusText="Fast";Dump("custom-static",d);Texts(u);
  ...
  var nd=Fan(nu);nd.SetToDefault();nd.GroupAddress=42;Dump("native-default",nd);Texts(nu);
  ...
  nd.LabelValueText="Ceiling";nd.StatusValueText="Stopped";nd.LowStatusText="Slow";nd.MedStatusText="Normal";nd.HighStatusText="Fast";Dump("native-custom",nd);Texts(nu);
  ...
  foreach(bool zero in new[]{false,true}){var other=Fan(Unit(true,zero));other.SetToDefault();Dump(zero?"opaque-blank-default":"opaque-default",other);}
 }
}
