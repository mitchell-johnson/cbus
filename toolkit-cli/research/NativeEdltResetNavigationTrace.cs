...
class NativeEdltResetNavigationTrace {
 ...
 static int snapshotDepth;
 static void Dump(string stage,EDLTUnit u){
  var before=u.PPAttributes.Select(p=>new {Attribute=p,Tokens=p.Values.ToArray(),Dirty=p.HasBeenChanged}).ToArray();bool flagBefore=PPAttribute.bInitialiseMode;Interlocked.Increment(ref snapshotDepth);
  ...
  Interlocked.Decrement(ref snapshotDepth);Require(before.All(p=>p.Tokens.SequenceEqual(p.Attribute.Values)&&p.Dirty==p.Attribute.HasBeenChanged)&&flagBefore==PPAttribute.bInitialiseMode,"Snapshot changed original raw tokens/dirty/flag");Out("snapshot-purity:"+stage+":raw-tokens-and-dirty-and-flag-unchanged=true");
 }
 ...
 [STAThread] static void Main(string[] args){
  ...
  using(var watchdog=new System.Threading.Timer(delegate{Out("fatal:whole-probe-deadline:90seconds:stage="+currentStage);Environment.Exit(4);},null,90000,Timeout.Infinite)){
   ...
   try{
    ...
    u.GetPPAttribute("UnitAddress").HasBeenChanged=true;u.GetPPAttribute("StaticTextString0").HasBeenChanged=true;var observedNav=u.GetPPAttribute("NavWidgetType");string priorNav=string.Join(" ",observedNav.Values);observedNav.PropertyChanged+=delegate(object sender,PropertyChangedEventArgs e){string now=string.Join(" ",observedNav.Values);if(now!=priorNav){string previous=priorNav;priorNav=now;Out("navigation-change:"+currentStage+":from="+previous+":to="+now+":snapshot-depth="+snapshotDepth+":stack="+Convert.ToBase64String(System.Text.Encoding.UTF8.GetBytes(new StackTrace(1,true).ToString())));}};Dump("input",u);stage=Mark("original-afterload");u.AfterLoadPPData();Dump("after-load",u);Pin(connection,peer,stage);
    ...
    if(args[3]=="handler"){
     ...
     stage=Mark("component-before-change");form.BeforeChangePpAttributes();Dump(stage,u);
    ...
    Dump("after-reset",u);Pin(connection,peer,stage);NoRenderer(form,"after-reset");Require(!form.IsDisposed,"Handler disposed form");Out("final-widget10:"+u.GetPPAttribute("Widget10WidgetType").ValueAsInt+":"+u.GetPPAttribute("Widget10WidgetByteValue1").ValueAsInt);
